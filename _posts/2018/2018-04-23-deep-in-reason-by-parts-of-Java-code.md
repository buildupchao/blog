...
Code:<br/>
   0:   iconst_1  // 把1压入栈顶<br/>
   1:   istore_1  // 把栈顶的值放到局部变量1（即i）中<br/>
   2:   iload_1   // 把i的值放到栈顶，也就是说此时栈顶的值是1<br/>
   3:   iinc    1, 1  //把局部变量1（即i）加1变为2，注意这时栈中仍然是1，没有改变<br/>
   6:   istore_1    //把栈顶的值放到局部变量1中，即i这时候由2变成了1<br/>

到此处，以上便完成了``` int i = 1; i = i++; ```这两行代码。

   7:   iconst_1   // 把1压入栈顶<br/>
   8:   istore_2   // 把栈顶的值放到局部变量1（即i）中<br/>
   9:   iinc    2, 1 // 把局部变量2（即j）加1变为2，注意这时栈中仍然是1，没有改变<br/>
   12:  iload_2    // 把局部变量2（即j）的值放到栈顶，此时栈顶的值变为2<br/>
   13:  istore_2   // 把栈顶的值放到局部变量2中，即j这时候真正由1变成了2<br/>
